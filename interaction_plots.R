...
makeplots <- function(plotname,pheno,SNP,exposure,minor_allele,major_allele)
{
  ...
  plotname <- enquo(plotname)
  pheno <- enquo(pheno)
  SNP <- enquo(SNP)
  exposure <- enquo(exposure)
  minor_allele <- enquo(minor_allele)
  major_allele <- enquo(major_allele)
  
  
  #Creates a dataframe with mean and sd of SHBG, grouped by allele genotype
  testdataframe <- summarise( group_by(sampledata, !!SNP),
                              mean_SHBG = mean(!!pheno),
                              se_SHBG = std.error(!!pheno))
  
  #Split data by exposure and genotype then apply mean and sd functions
  exposure0 <- summarise( group_by(filter(sampledata, !!exposure == 0), !!SNP),
                          mean_SHBG = mean(!!pheno),
                          se_SHBG = std.error(!!pheno))
  exposure1 <- summarise( group_by(filter(sampledata, !!exposure == 1), !!SNP),
                          mean_SHBG = mean(!!pheno),
                          se_SHBG = std.error(!!pheno))
  ...
  #SNP Allele Naming
  homo_minor <- paste(quo_name(minor_allele),
                      quo_name(minor_allele),
                      sep = "")
  
  het <- paste(quo_name(minor_allele),
               quo_name(major_allele),
               sep = "")
  
  homo_major <- paste(quo_name(major_allele),
                      quo_name(major_allele),
                      sep = "")
  
  #2nd Plot Re-leveling
  sampledatachar <- mutate(sampledata, Genotype = factor(pull(sampledata, !!SNP),
                                                         levels = c(0,1,2),
                                                         labels = c(homo_major, het, homo_minor)))
  ...
  signif1 <- max(exposure$mean_SHBG[1:3]) + 
    max(exposure$se_SHBG[1:3]) + 
    0.09*bound_diff2
  signif2 <- max(exposure$mean_SHBG[1:3]) + 
    max(exposure$se_SHBG[1:3]) + 
    0.18*bound_diff2
  signif3 <- max(exposure$mean_SHBG[1:3]) +
    max(exposure$se_SHBG[1:3]) + 
    0.27*bound_diff2
  signif4 <- max(exposure$mean_SHBG[4:6]) + 
    max(exposure$se_SHBG[4:6]) + 
    0.09*bound_diff2
  signif5 <- max(exposure$mean_SHBG[4:6]) + 
    max(exposure$se_SHBG[4:6]) + 
    0.18*bound_diff2
  signif6 <- max(exposure$mean_SHBG[4:6]) + 
    max(exposure$se_SHBG[4:6]) + 
    0.27*bound_diff2
  ...
  plot_final <- annotate_figure(plot_final, top = text_grob(quo_name(plotname)))
  print(plot_final)
